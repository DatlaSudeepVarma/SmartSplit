...
import { Moon, Sun, ArrowLeftRight, Calculator as CalcIcon, Shield, Menu, X } from 'lucide-react';
...
const Navbar = ({ variant = "fixed" }: NavbarProps) => {
    ...
    const toolButtons = (
        <>
            <button type="button" onClick={() => setShowCalc(true)} className={iconBtn} title="Calculator">
                <CalcIcon size={16} className="shrink-0" />
            </button>
            <button type="button" onClick={() => setShowConv(true)} className={iconBtn} title="Currency Converter">
    ...
    const currencySelect = (
        <label className="flex items-center gap-1 text-[11px] font-bold text-gray-700 dark:text-white/70">
            <span className="hidden uppercase tracking-wide text-gray-500 sm:inline dark:text-white/45">CCY</span>
            <select
                value={currency}
                onChange={(e) => setCurrency(e.target.value as Currency)}
                className="max-w-[3.25rem] cursor-pointer appearance-none bg-transparent py-0.5 pr-0.5 text-[11px] font-black text-gray-800 outline-none transition-all hover:text-brand-green dark:text-white/90 dark:hover:text-[#d4ff00]"
            >
    ...
    const themeToggle = (
        <button
            type="button"
            onClick={toggleTheme}
            className="rounded-full p-1.5 text-gray-600 transition-all hover:bg-black/[0.05] dark:text-white/75 dark:hover:bg-white/10"
    ...
    const navClassName = [
        ...
        isContained
            ? 'sticky top-0 z-50 mx-auto w-full max-w-[1000px] rounded-b-2xl border-x border-b px-5 py-2.5 sm:px-8 sm:py-3'
            : (isInset
                ? 'relative mx-auto w-[min(92vw,42rem)] rounded-full border px-6 py-2.5 backdrop-blur-md sm:py-3'
                : 'relative mx-auto max-w-lg rounded-xl border px-2 py-1.5 backdrop-blur-xl sm:max-w-xl sm:rounded-2xl sm:px-3 sm:py-2 dark:backdrop-blur-2xl'),
        shellClass,
    ].join(' ');
            ...
            {splashFinished && (
                ...
                >
                    ...
                    >
                        ...
                        <div className="flex min-w-0 max-w-[42%] flex-1 items-center justify-start gap-0.5 sm:max-w-none sm:gap-1">
                            <div className="flex items-center rounded-lg border border-gray-200/60 bg-gray-50/90 p-0.5 dark:border-white/10 dark:bg-white/[0.06]">
                                {toolButtons}
                            </div>
                            <div className="hidden h-4 w-px bg-gray-200/90 sm:block dark:bg-white/15" />
                            {currencySelect}
                        </div>
                        ...
                        <div className="absolute left-1/2 top-1/2 flex -translate-x-1/2 -translate-y-1/2 justify-center">
                            ...
                            >
                                <SmartSplitLogo className="h-6 w-6 shrink-0 sm:h-7 sm:w-7" />
                                <span className="font-mier text-xs font-semibold tracking-tight text-gray-900 dark:text-white sm:text-sm">
                                    SmartSplit
                                </span>
                            </button>
                        </div>
                        ...
                        <div className="flex min-w-0 max-w-[42%] flex-1 items-center justify-end gap-0.5 sm:max-w-none sm:gap-1">
                            ...
                            {user ? (
                                <>
                                    {user.isAdmin && (
                                        <button
                                            ...
                                            className="rounded-md p-1.5 text-gray-500 hover:bg-purple-500/10 hover:text-purple-500 dark:text-gray-400"
                                            title="Admin"
                                        >
                                            <Shield size={16} />
                                        </button>
                                    ...
                                    {!user.isAdmin && (
                                        <button
                                            ...
                                            className="flex h-7 w-7 items-center justify-center rounded-md bg-gradient-to-tr from-brand-blue to-brand-orange text-[10px] font-black text-white shadow-sm sm:h-8 sm:w-8 sm:text-xs"
                                        >
                                ...
                                <button
                                    ...
                                    className="rounded-lg px-2.5 py-1 text-xs font-semibold text-gray-800 transition-colors hover:bg-black/[0.05] dark:text-white dark:hover:bg-white/10 sm:px-3 sm:text-sm"
                                >
                                    Sign in
                                </button>
                            )}
                            <button
                                type="button"
                                className="rounded-lg p-1.5 text-gray-800 hover:bg-black/[0.05] sm:hidden dark:text-white dark:hover:bg-white/10"
                ...
                {splashFinished && mobileOpen && (
                    <div className="pointer-events-none fixed inset-0 z-[110] sm:hidden">
                        ...
                        <motion.div
                            ...
                            className={`pointer-events-auto absolute left-4 right-4 mx-auto max-w-lg overflow-hidden rounded-xl border border-gray-200/80 bg-white/95 p-3 shadow-xl backdrop-blur-xl dark:border-white/12 dark:bg-[#0c0c0c]/95 sm:left-6 sm:right-6 ${isInset ? 'top-[calc(0.75rem+3.5rem+0.75rem)] sm:top-[calc(1rem+3.5rem+0.75rem)]' : 'top-[3.75rem]'}`}
                        >
                            ...
                            <div className="flex items-center justify-between gap-2 border-b border-gray-200/60 py-3 dark:border-white/10">
                                ...
                                <div className="flex items-center gap-1 rounded-lg border border-gray-200/50 bg-gray-100/90 p-0.5 dark:border-white/10 dark:bg-white/[0.06]">
                                    {toolButtons}
                                </div>
                            </div>
