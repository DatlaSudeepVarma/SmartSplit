...
const ModuleCard = ({ title, desc, icon: Icon, gif, onClick, disabled }: ModuleCardProps) => (
    <m.div
        ...
        whileHover={!disabled ? { y: -6, transition: { duration: 0.25 } } : undefined}
        ...
        className={`relative overflow-hidden text-left p-7 sm:p-8 rounded-3xl transition-all duration-300
            ...
            ${disabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer group hover:shadow-[0_28px_70px_-40px_rgba(0,0,0,0.35)] hover:border-brand-blue/30'}
        `}
    >
        <div
